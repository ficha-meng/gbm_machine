...
public:
  ...
  float sum_gradients = 0.0;
  /*! \brief Sum of hessians on this bin */
  float sum_hessians = 0.0;
...
public:
  ...
  */
  void FindBin(std::vector<float>* values, size_t total_sample_cnt, int max_bin, BinType bin_type);
...
private:
  ...
  std::vector<float> bin_upper_bound_;
...
public:
  ...
  */
  static Bin* CreateBin(data_size_t num_data, int num_bin,
    float sparse_rate, bool is_enable_sparse, 
