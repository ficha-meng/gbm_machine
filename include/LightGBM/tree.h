...
public:
  ...
  */
  ...
  std::string ToString();
  std::string ToStringFicha(const char *filename, int tree_num, int idx, int loop, int max_num_leaves, int shift_bit);
  std::string ToStringFicha2(const char *filename, int tree_num, int idx, int loop, int max_num_leaves, int shift_bit);
  ...
  static std::vector<std::function<bool(float, float)>> decision_funs;
...
private:
  ...
  */
  ...
  std::vector<float> internal_value_;
...
inline int Tree::GetLeaf(const float* feature_values) const {
  ...
  while (node >= 0) 
  {
	//printf("%d\n", split_feature_real_[node]);
	bool decision = decision_funs[decision_type_[node]](feature_values[split_feature_real_[node]], threshold_[node]);
	if (decision)
	{
	// printf("left!\n");
      node = left_child_[node];
