...
//template<typename T>
inline static std::string ArrayToStringFicha(const std::vector<float>& arr, size_t n, std::string delimiter, int max, int idx, int shift_bit) {
	if (arr.empty() || n == 0) {
		return std::string("");
	}

	std::stringstream str_buf;
	short arrshort = round(arr[0] * (1 << shift_bit));
	str_buf << arrshort;
	for (size_t i = 1; i < std::min(n, arr.size()); ++i) {
		str_buf << delimiter;
		arrshort = round(arr[i] * (1 << shift_bit));
		str_buf << arrshort;
	}
	//if (idx!=max-1)
	//str_buf << delimiter;
...
inline static std::string ArrayToStringFicha(const std::vector<int>& arr, size_t n, std::string delimiter, int max, int idx) {
	if (arr.empty() || n == 0) {
		return std::string("");
	}
	//
...
inline static std::string ArrayToStringFicha(const std::vector<short>& arr, size_t n, std::string delimiter, int max, int idx) {
	if (arr.empty() || n == 0) {
		return std::string("");
	}
...
template<typename T>
inline static std::vector<T> StringToArrayFicha(const std::string& str, char delimiter, size_t n) {
	std::vector<std::string> strs = Split(str.c_str(), delimiter);
	if (strs.size() != n) {
		Log::Fatal("StringToArray error, size doesn't match.");
	}
