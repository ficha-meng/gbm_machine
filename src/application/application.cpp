...
void Application::LoadData() {
  ...
  Log::Info("Finished loading data in %f seconds",
    std::chrono::duration<float, std::milli>(end_time - start_time) * 1e-3);
}
  ...
  if (config_.is_parallel) {
    ...
    if (config_.boosting_type == BoostingType::kGBDT || config_.boosting_type == BoostingType::kDART) {
      ...
      config_.boosting_config.tree_config.feature_fraction =
        GlobalSyncUpByMin<float>(config_.boosting_config.tree_config.feature_fraction);
    }
  }
  // create boosting
  boosting_.reset(
...
void Application::Train() {
  ...
  for (int iter = 0; iter < total_iter && !is_finished; ++iter) {
    ...
    Log::Info("%f seconds elapsed, finished iteration %d", std::chrono::duration<float,
      std::milli>(end_time - start_time) * 1e-3, iter + 1);
  }
  // save model to file
  int shift_bit = config_.io_config.shift_bit;
  int max_num_leaves = config_.boosting_config.tree_config.num_leaves;
  int width = config_.io_config.modelwidth;
  int height = config_.io_config.modelheight;

  std::string filename = config_.io_config.output_model_path + config_.io_config.output_model + std::to_string(config_.io_config.trainiteration);
  //filename = filename + ".txt";
  //boosting_->SaveModelToFile(-1, filename.c_str());

  printf("Waiting for save model to file!\n");
  if (config_.objective_type == "multiclass")
  {
	  std::string filenameficha = config_.io_config.output_model + std::to_string(config_.io_config.trainiteration);
	  boosting_->SaveModelToFileFicha(-1, config_.io_config.output_model_path.c_str(), filenameficha.c_str(), max_num_leaves, shift_bit, width, height);
	  boosting_->SaveModelToFileFicha(-1, config_.io_config.output_model_path.c_str(), config_.io_config.output_model.c_str(), max_num_leaves, shift_bit, width, height);
  }
  if (config_.objective_type == "regression")
  {
	  size_t format= config_.io_config.data_filename.find_last_of(".");
	  std::string modelname = config_.io_config.data_filename.substr(0,format);
	  std::string filenameficha = config_.io_config.output_model + std::to_string(config_.io_config.trainiteration);
	  boosting_->SaveModelToFileFicha(-1, config_.io_config.output_model_path.c_str(), filenameficha.c_str(), max_num_leaves, shift_bit, width, height);
	  boosting_->SaveModelToFileFicha(-1, config_.io_config.output_model_path.c_str(), config_.io_config.output_model.c_str(), max_num_leaves, shift_bit, width, height);
  }
