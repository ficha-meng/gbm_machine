...
#pragma omp master
    ...
    for (int i = 0; i < num_threads_; ++i) {
      features_.push_back(std::vector<float>(num_features_));
    }
    features_.shrink_to_fit();
    if (is_predict_leaf_index) {
      predict_fun_ = [this](const std::vector<std::pair<int, float>>& features) {
...
#pragma omp parallel for schedule(static) private(oneline_features)
      for (data_size_t i = 0; i < static_cast<data_size_t>(lines.size()); ++i) {
        ...
        // predict
        pred_result[i] = Common::Join<float>(predict_fun_(oneline_features), "\t");
      }
...
private:
  int PutFeatureValuesToBuffer(const std::vector<std::pair<int, float>>& features) {
    ...
    // init feature value
    std::memset(features_[tid].data(), 0, sizeof(float)*num_features_);
    // put feature value
