...
public:
  ...
  */
  void ResetShrinkageRate(float shrinkage_rate) override {
  ...
  */
  std::vector<float> GetEvalAt(int data_idx) const override;
  ...
  */
  std::vector<float> PredictRaw(const float* feature_values) const override;
  ...
  * \return Prediction result for this record
  */
  std::vector<float> Predict(const float* feature_values) const override;
  ...
  */
  std::vector<int> PredictLeafIndex(const float* value) const override;
  ...
  */
  ...
  virtual void SaveModelToFileFicha(int num_iterations, const char* path, const char* filename, int max_num_leaves, int shift_bit,int width,int height) const override;
  /*!
