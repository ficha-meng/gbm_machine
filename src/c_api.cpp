...
public:
  ...
  std::vector<float> Predict(const std::vector<std::pair<int, float>>& features) {
  ...
  void SaveModelToFile(int num_iteration, const char* filename) {
	...
	//boosting_->SaveModelToFileFicha(num_iteration, filename);
  }

  void SaveModelToFileFicha(int num_iteration, const char* path, const char* filename, int max_num_leaves, int shift_bit,int width,int height) {
	  // boosting_->SaveModelToFile(num_iteration, filename);
	  boosting_->SaveModelToFileFicha(num_iteration, path, filename,max_num_leaves,shift_bit,width,height);
  }
...
DllExport int LGBM_DatasetCreateFromMat(const void* data,
  ...
  if (reference == nullptr) {
    ...
    std::vector<std::vector<float>> sample_values(ncol);
...
DllExport int LGBM_DatasetCreateFromCSR(const void* indptr,
  ...
  if (reference == nullptr) {
    ...
    for (size_t i = 0; i < sample_indices.size(); ++i) {
      auto idx = sample_indices[i];
      auto row = get_row_fun(static_cast<int>(idx));
      for (std::pair<int, float>& inner_data : row) {
...
DllExport int LGBM_DatasetCreateFromCSC(const void* col_ptr,
  ...
  if (reference == nullptr) {
    ...
    const int sample_cnt = static_cast<int>(nrow < io_config.bin_construct_sample_cnt ? nrow : io_config.bin_construct_sample_cnt);
    auto sample_indices = rand.Sample(nrow, sample_cnt);
    std::vector<std::vector<float>> sample_values(ncol_ptr - 1);
#pragma omp parallel for schedule(guided)
...
DllExport int LGBM_BoosterSaveModel(BoosterHandle handle,
  int num_iteration,
  const char* path, const char* filename, int max_num_leaves, int shift_bit,int width,int height) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  //ref_booster->SaveModelToFile(num_iteration, filename);
  ref_booster->SaveModelToFileFicha(num_iteration, path, filename, max_num_leaves,shift_bit,width,height);
  API_END();
}
...
std::function<std::vector<float>(int row_idx)>
RowFunctionFromDenseMatric(const void* data, int num_row, int num_col, int data_type, int is_row_major) {
  if (data_type == C_API_DTYPE_FLOAT32) {
    const float* data_ptr = reinterpret_cast<const float*>(data);
    if (is_row_major) {
      return [data_ptr, num_col, num_row](int row_idx) {
        std::vector<float> ret(num_col);
        auto tmp_ptr = data_ptr + num_col * row_idx;
        for (int i = 0; i < num_col; ++i) {
          ret[i] = static_cast<float>(*(tmp_ptr + i));
        }
        return ret;
      };
    } else {
      return [data_ptr, num_col, num_row](int row_idx) {
        std::vector<float> ret(num_col);
        for (int i = 0; i < num_col; ++i) {
          ret[i] = static_cast<float>(*(data_ptr + num_row * i + row_idx));
        }
        return ret;
      };
    }
...
std::function<std::vector<std::pair<int, float>>(int row_idx)>
...
std::function<std::vector<std::pair<int, float>>(int idx)>
...
std::vector<float> SampleFromOneColumn(const std::vector<std::pair<int, float>>& data, const std::vector<int>& indices) {
  size_t j = 0;
