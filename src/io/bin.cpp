...
void BinMapper::FindBin(std::vector<float>* values, size_t total_sample_cnt, int max_bin, BinType bin_type) {
  bin_type_ = bin_type;
  ...
  if (bin_type_ == BinType::NumericalBin) {
    if (num_values <= max_bin) {
      ...
      num_bin_ = num_values;
      bin_upper_bound_ = std::vector<float>(num_values);
      ...
      mean_bin_size = rest_sample_cnt / static_cast<float>(rest_bin_cnt);
      ...
      for (int i = 0; i < num_values - 1; ++i) {
        ...
        if (is_big_count_value[i] || cur_cnt_inbin >= mean_bin_size ||
          (is_big_count_value[i + 1] && cur_cnt_inbin >= std::max(1.0f, mean_bin_size * 0.5f))) {
      ...
      bin_upper_bound_ = std::vector<float>(bin_cnt);
      num_bin_ = bin_cnt;
    ...
    if (used_cnt / static_cast<float>(sample_size) < 0.95f) {
...
void BinMapper::CopyTo(char * buffer) {
  ...
  if (bin_type_ == BinType::NumericalBin) {
    std::memcpy(buffer, bin_upper_bound_.data(), num_bin_ * sizeof(float));
  } else {
...
void BinMapper::CopyFrom(const char * buffer) {
  ...
  buffer += sizeof(bin_type_);
  if (bin_type_ == BinType::NumericalBin) {
    bin_upper_bound_ = std::vector<float>(num_bin_);
    std::memcpy(bin_upper_bound_.data(), buffer, num_bin_ * sizeof(float));
  } else {
    bin_2_categorical_ = std::vector<int>(num_bin_);
...
void BinMapper::SaveBinaryToFile(FILE* file) const {
  ...
  if (bin_type_ == BinType::NumericalBin) {
    fwrite(bin_upper_bound_.data(), sizeof(float), num_bin_, file);
  } else {
...
Bin* Bin::CreateBin(data_size_t num_data, int num_bin, float sparse_rate, 
