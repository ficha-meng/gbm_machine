...
void IOConfig::Set(const std::unordered_map<std::string, std::string>& params) {
  ...
  GetInt(params,"shift_bit",&shift_bit); // for ficha short 
  GetInt(params, "modelwidth", &modelwidth); // for ficha width
  GetInt(params, "modelheight", &modelheight); // for ficha height
  GetInt(params, "LearnLoop", &trainiteration);
  ...
  GetString(params, "output_model_path", &output_model_path);
...
void ObjectiveConfig::Set(const std::unordered_map<std::string, std::string>& params) {
  ...
  GetInt(params, "num_class", &num_class);
  CHECK(num_class >= 1);
  Getfloat(params, "scale_pos_weight", &scale_pos_weight);
  std::string tmp_str = "";
  if (GetString(params, "label_gain", &tmp_str)) {
    label_gain = Common::StringToArray<float>(tmp_str, ',');
  } else {
...
void TreeConfig::Set(const std::unordered_map<std::string, std::string>& params) {
  ...
  Getfloat(params, "lambda_l1", &lambda_l1);
  CHECK(lambda_l1 >= 0.0f)
  Getfloat(params, "lambda_l2", &lambda_l2);
  CHECK(lambda_l2 >= 0.0f)
  Getfloat(params, "min_gain_to_split", &min_gain_to_split);
...
void BoostingConfig::Set(const std::unordered_map<std::string, std::string>& params) {
  ...
  Getfloat(params, "drop_rate", &drop_rate);
