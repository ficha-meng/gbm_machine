...
Dataset* DatasetLoader::CostructFromSampleData(std::vector<std::vector<float>>& sample_values, size_t total_sample_size, data_size_t num_data) {
...
#pragma omp parallel for schedule(guided) private(oneline_features) firstprivate(tmp_label)
    for (data_size_t i = 0; i < dataset->num_data_; ++i) {
      ...
      parser->ParseOneLine(text_data[i].c_str(), &oneline_features, &tmp_label);
      // set initial score
      std::vector<float> oneline_init_score = predict_fun_(oneline_features);
