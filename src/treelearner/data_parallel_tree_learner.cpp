...
void DataParallelTreeLearner::BeforeTrain() {
  ...
  std::tuple<data_size_t, float, float> data(smaller_leaf_splits_->num_data_in_leaf(),
  ...
  Network::Allreduce(input_buffer_.data(), size, size, output_buffer_.data(), [](const char *src, char *dst, int len) {
    int used_size = 0;
    int type_size = sizeof(std::tuple<data_size_t, float, float>);
    const std::tuple<data_size_t, float, float> *p1;
    std::tuple<data_size_t, float, float> *p2;
    while (used_size < len) {
      p1 = reinterpret_cast<const std::tuple<data_size_t, float, float> *>(src);
      p2 = reinterpret_cast<std::tuple<data_size_t, float, float> *>(dst);
