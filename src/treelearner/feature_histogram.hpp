...
public:
  ...
  */
  void Construct(const data_size_t* data_indices, data_size_t num_data, float sum_gradients,
    float sum_hessians, const score_t* ordered_gradients, const score_t* ordered_hessians) {
  ...
  * \param hessian
  */
  void Construct(const OrderedBin* ordered_bin, int leaf, data_size_t num_data, float sum_gradients,
    float sum_hessians, const score_t* gradients, const score_t* hessians) {
    std::memset(data_.data(), 0, sizeof(HistogramBinEntry)* num_bins_);
    num_data_ = num_data;
    sum_gradients_ = sum_gradients;
    sum_hessians_ = sum_hessians + 2 * kEpsilon;
  ...
  * \param sum_hessians sum of hessians of current leaf
  */
  void SetSumup(data_size_t num_data, float sum_gradients, float sum_hessians) {
    num_data_ = num_data;
    sum_gradients_ = sum_gradients;
    sum_hessians_ = sum_hessians + 2 * kEpsilon;
  }
  ...
  */
  ...
  void FindBestThresholdForNumerical(SplitInfo* output) {
    float best_sum_left_gradient = NAN;
    float best_sum_left_hessian = NAN;
    ...
    float sum_right_hessian = kEpsilon;
    data_size_t right_count = 0;
    float gain_shift = GetLeafSplitGain(sum_gradients_, sum_hessians_);
    float min_gain_shift = gain_shift + tree_config_->min_gain_to_split;
    ...
    for (unsigned int t = num_bins_ - 1; t > 0; --t) {
      ...
      float sum_left_hessian = sum_hessians_ - sum_right_hessian;
      // if sum hessian too small
      if (sum_left_hessian < tree_config_->min_sum_hessian_in_leaf) break;

      float sum_left_gradient = sum_gradients_ - sum_right_gradient;
      // current split gain
      float current_gain = GetLeafSplitGain(sum_left_gradient, sum_left_hessian)
  ...
  */
  void FindBestThresholdForCategorical(SplitInfo* output) {
    ...
    for (int t = num_bins_ - 1; t >= 0; --t) {
      float sum_current_gradient = data_[t].sum_gradients;
      float sum_current_hessian = data_[t].sum_hessians;
      ...
      float sum_other_hessian = sum_hessians_ - sum_current_hessian;
      // if sum hessian too small
      if (sum_other_hessian < tree_config_->min_sum_hessian_in_leaf) continue;

      float sum_other_gradient = sum_gradients_ - sum_current_gradient;
      // current split gain
      float current_gain = GetLeafSplitGain(sum_other_gradient, sum_other_hessian) 
...
private:
  ...
  * \return split gain
  */
  float GetLeafSplitGain(float sum_gradients, float sum_hessians) const {
    float abs_sum_gradients = std::fabs(sum_gradients);
    if (abs_sum_gradients > tree_config_->lambda_l1) {
      float reg_abs_sum_gradients = abs_sum_gradients - tree_config_->lambda_l1;
  ...
  * \return leaf output
  */
  float CalculateSplittedLeafOutput(float sum_gradients, float sum_hessians) const {
    float abs_sum_gradients = std::fabs(sum_gradients);
    if (abs_sum_gradients > tree_config_->lambda_l1) {
