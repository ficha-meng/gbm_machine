...
public:
  ...
  * \param sum_hessians
  */
  void Init(int leaf, const DataPartition* data_partition, float sum_gradients, float sum_hessians) {
    leaf_index_ = leaf;
  ...
  */
  void Init(const score_t* gradients, const score_t* hessians) {
    ...
    leaf_index_ = 0;
    data_indices_ = nullptr;
    float tmp_sum_gradients = 0.0f;
    float tmp_sum_hessians = 0.0f;
...
#pragma omp parallel for schedule(static) reduction(+:tmp_sum_gradients, tmp_sum_hessians)
  ...
  * \param sum_hessians
  */
  void Init(float sum_gradients, float sum_hessians) {
    leaf_index_ = 0;
    sum_gradients_ = sum_gradients;
    sum_hessians_ = sum_hessians;
    for (SplitInfo& split_info : best_split_per_feature_) {
  ...
  */
  ...
  float sum_gradients() const { return sum_gradients_; }
  
  /*! \brief Get sum of hessians of current leaf */
  float sum_hessians() const { return sum_hessians_; }
